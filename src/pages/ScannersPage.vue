<template>
  <q-page class="scanners-page" padding>

    <q-card class="main-card">
      <!-- هدر کارت -->
      <q-card-section class="card-header">
        <div class="header-content">
          <q-icon name="scanner" class="header-icon" />
          <div class="header-text">
            <div class="text-h5">مدیریت اسکنرها</div>
            <div class="text-subtitle">مدیریت و ویرایش اطلاعات اسکنرها</div>
          </div>
        </div>
      </q-card-section>
      ...
      <q-card-section class="content-section">
        ...
        <div class="action-bar">
          ...
          <q-input
            ...
            debounce="500"
            v-model="search"
            placeholder="جستجوی مدل اسکنر یا مشخصات..."
            @update:model-value="loadScanners"
            class="search-input"
          >
    ...
    <q-dialog v-model="dialog" persistent>
      <q-card class="dialog-card scanner-dialog">
        <q-card-section class="dialog-header">
          <div class="dialog-title">
            <q-icon :name="isEdit ? 'edit' : 'add'" class="dialog-icon" />
            {{ isEdit ? 'ویرایش اسکنر' : 'افزودن اسکنر جدید' }}
          </div>
        </q-card-section>
        <!-- پیام خطا -->
        <q-banner
        ...
        <q-card-section class="dialog-body">
          ...
          <div class="row q-col-gutter-md form-row">
            <div class="col-6">
              ...
              >
                <template v-slot:prepend>
                  <q-icon name="scanner" class="field-icon" />
                </template>
              </q-input>
            </div>
            <div class="col-6">
    ...
    <q-dialog v-model="viewDialog">
      <q-card class="view-dialog-card">
        <q-card-section class="view-header">
          ...
          <div class="view-title">مشاهده اطلاعات اسکنر</div>
        </q-card-section>

        <q-card-section class="view-body">
          <div class="row q-col-gutter-md">
            <div class="col-6">
              <q-input v-model="viewData.model" label="مدل اسکنر" outlined dense readonly />
            </div>
            <div class="col-6">
              <q-input v-model="viewData.quantity" label="تعداد" outlined dense readonly />
            </div>
            <div class="col-6">
              <q-input v-model="viewData.type" label="نوع اسکنر" outlined dense readonly />
            </div>
            <div class="col-6">
    ...
    <q-dialog v-model="deleteDialog">
      <q-card class="delete-dialog-card">
        ...
        <q-card-section class="delete-body">
          آیا از حذف اسکنر "{{ selectedScanner.model }}" اطمینان دارید؟
...
async function saveScanner() {
  ...
  if (!form.value.model || !form.value.branch_id ) {
    errorMessage.value = 'لطفاً فیلدهای مدل اسکنر، شعبه را پر کنید'
    return
  }
  ...
  try {
    if (isEdit.value) {
      await api.put(`/scanners/${form.value.id}`, submitData)
    } else {
      await api.post('/scanners', submitData)
    }
    dialog.value = false
    loadScanners()
  } catch (err) {
    console.error('خطا در ثبت اسکنر:', err)
...
function confirmDelete(row) {
  selectedScanner.value = row
  deleteDialog.value = true
}

async function deleteScanner() {
  try {
    await api.delete(`/scanners/${selectedScanner.value.id}`)
    loadScanners()
...
.scanners-table :deep(.q-table__top) {
  background: #f8fbff;
  border-bottom: 1px solid #e3f2fd;
}

.scanners-table :deep(.q-table__control) {
  direction: rtl;
}
